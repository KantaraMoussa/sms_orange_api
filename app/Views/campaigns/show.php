<?php if (!$campagne): ?>
    <div class="alert alert-danger">Campagne introuvable.</div>
<?php return; endif; ?>
<hr>
<div class="row mb-3">
    <div class="col-12">
        <div class="card">
            <div class="card-header d-flex justify-content-between align-items-center">
                ...
                <div>
                    <?php if ($isDraft && $hasRecipients): ?>
                        <form method="post" action="<?= route('campaigns.launch') ?>" class="d-inline" onsubmit="return confirm('Lancer l\'envoi de <?= (int)$campagne['total_destinataires'] ?> destinataire(s) (<?= (int) $smsNeeded ?> SMS estimés) maintenant ?');">
                            <?= csrf_field() ?>
                    ...
                    <?php elseif ($isScheduled): ?>
                        <form method="post" action="<?= route('campaigns.launch') ?>" class="d-inline" onsubmit="return confirm('Lancer l\'envoi maintenant plutôt que d\'attendre la date programmée ?');">
                            <?= csrf_field() ?>
                            <input type="hidden" name="campagne_id" value="<?= $campagne['id'] ?>">
                        ...
                        <form method="post" action="<?= route('campaigns.unschedule') ?>" class="d-inline">
                            <?= csrf_field() ?>
                            <input type="hidden" name="campagne_id" value="<?= $campagne['id'] ?>">
                    ...
                    <?php elseif ($campagne['statut'] === 'RUNNING'): ?>
                        <form method="post" action="<?= route('campaigns.pause') ?>" class="d-inline">
                            <?= csrf_field() ?>
                            <input type="hidden" name="campagne_id" value="<?= $campagne['id'] ?>">
                            <button type="submit" name="pause_campagne" class="btn btn-warning">⏸ Pause</button>
                        </form>
                    <?php elseif ($campagne['statut'] === 'PAUSED'): ?>
                        <form method="post" action="<?= route('campaigns.resume') ?>" class="d-inline">
                            <?= csrf_field() ?>
                            <input type="hidden" name="campagne_id" value="<?= $campagne['id'] ?>">
                    ...
                    <?php if ($isActive || $campagne['statut'] === 'PAUSED'): ?>
                        <form method="post" action="<?= route('campaigns.cancel') ?>" class="d-inline" onsubmit="return confirm('Annuler cette campagne ? Les SMS déjà envoyés resteront dans l\'historique.');">
                            <?= csrf_field() ?>
                            <input type="hidden" name="campagne_id" value="<?= $campagne['id'] ?>">
            ...
            <div class="card-body">
                ...
                <form method="post" action="<?= route('campaigns.retryFailures') ?>" class="mb-3" onsubmit="return confirm('Réessayer les <?= (int)$campagne['nombre_echecs'] ?> échec(s) rattrapables ?');">
                    <?= csrf_field() ?>
                    <input type="hidden" name="campagne_id" value="<?= $campagne['id'] ?>">
                ...
                <div class="row g-3 mb-3">
                    ...
                    <div class="col-md-4">
                        <form method="post" action="<?= route('campaigns.sendTestSms') ?>" class="d-flex gap-1">
                            <?= csrf_field() ?>
                            <input type="hidden" name="campagne_id" value="<?= $campagne['id'] ?>">
                ...
                <div class="alert alert-info d-flex justify-content-between align-items-center mb-0">
                    ...
                    <form method="post" action="<?= route('campaigns.stopRecurrence') ?>" class="m-0" onsubmit="return confirm('Arrêter le renouvellement automatique de cette campagne ?');">
                        <?= csrf_field() ?>
                        <input type="hidden" name="campagne_id" value="<?= $campagne['id'] ?>">
...
<div class="modal fade" id="scheduleModal<?= $campagne['id'] ?>" tabindex="-1" aria-labelledby="scheduleModalLabel<?= $campagne['id'] ?>" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content shadow-lg">
            ...
            <form action="<?= route('campaigns.schedule') ?>" method="POST">
                <?= csrf_field() ?>
                <input type="hidden" name="campagne_id" value="<?= $campagne['id'] ?>">
...
<div class="modal fade" id="composeModal<?= $campagne['id'] ?>" tabindex="-1" aria-labelledby="composeModalLabel<?= $campagne['id'] ?>" aria-hidden="true">
    <div class="modal-dialog modal-lg">
        <div class="modal-content shadow-lg">
            ...
            <form action="<?= route('campaigns.addRecipientsFromAudience') ?>" method="POST" id="composeForm<?= $campagne['id'] ?>">
                <?= csrf_field() ?>
                <input type="hidden" name="campagne_id" value="<?= $campagne['id'] ?>">
                <div class="modal-body">
                    <div class="mb-3">
...
function updateCounterAndPreview<?= $campagne['id'] ?>() {
    ...
    debounceTimer<?= $campagne['id'] ?> = setTimeout(function () {
        ...
        fetch('<?= route('campaigns.previewTools') ?>&' + params.toString())
            .then(r => r.json())
            .then(data => {
...
<div class="modal fade" id="importExcelModal<?= $campagne['id'] ?>" tabindex="-1" aria-labelledby="importExcelModalLabel<?= $campagne['id'] ?>" aria-hidden="true">
    <div class="modal-dialog modal-lg">
        <div class="modal-content shadow-lg">
            ...
            <div class="modal-body">
                <form action="<?= route('campaigns.importExcel') ?>" method="POST" enctype="multipart/form-data">
                    <?= csrf_field() ?>
                    <input type="hidden" name="campagne_id" value="<?= $campagne['id'] ?>">
                    <div class="mb-3">
...
(function () {
    ...
    function poll() {
        fetch('<?= route('campaigns.poll') ?>&campagne_id=' + campagneId)
            .then(r => r.json())
            .then(data => {
