...
if (!$group) {
    echo '<div class="alert alert-danger mt-3">Groupe introuvable.</div>';
    return;
}
$memberContacts = contacts()->allContacts($groupeId);
...
<div class="row mt-3">
    <div class="col-12 mb-3">
        <a href="?page=groupe" class="btn btn-sm btn-outline-secondary">« Retour aux groupes</a>
    </div>
    <div class="col-lg-7">
        ...
        <div class="card mt-3">
            ...
            <div class="card-body">
                <form action="../server/app.php" method="post" enctype="multipart/form-data" class="row g-2 align-items-end">
                    <?= csrf_field() ?>
                    <input type="hidden" name="groupe_id" value="<?= $groupeId ?>">
                    <div class="col-md-8">
    ...
    <div class="col-lg-5">
        <div class="card card-default">
            ...
            <div class="card-body">
                <form action="../server/app.php" method="post" onsubmit="return confirm('Créer une campagne pour les <?= count($memberContacts) ?> contact(s) de ce groupe ?');">
                    <?= csrf_field() ?>
                    <input type="hidden" name="groupe_id" value="<?= $groupeId ?>">
                    <div class="mb-3">
        ...
        <div class="modal-content shadow-lg">
            ...
            <div class="modal-body">
                <?php if (empty($availableToAdd)): ?>
                    <p class="text-muted">Tous vos contacts sont déjà dans ce groupe, ou vous n'avez encore aucun contact. <a href="?page=contacts">Ajouter des contacts</a>.</p>
                <?php else: ?>
                <form action="../server/app.php" method="post">
                    <?= csrf_field() ?>
                    <input type="hidden" name="groupe_id" value="<?= $groupeId ?>">
