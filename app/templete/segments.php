<?php
// Segments dynamiques (cahier des charges V2.0 §13) : un segment est un
// filtre réutilisable comme audience de campagne, jamais une copie de
// contacts — le nombre affiché ici est recalculé à chaque chargement de la
// page (segments()->countContacts()), pas figé à la création du segment.
                        ...
                        <tbody>
                            <?php foreach ($segmentsList as $s): ?>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <form method="post" action="../server/app.php" class="d-inline" onsubmit="return confirm('Supprimer le segment « <?= htmlspecialchars(addslashes($s['nom'])) ?> » ?');">
                                            <?= csrf_field() ?>
...
<div class="modal fade" id="createSegmentModal" tabindex="-1" aria-labelledby="createSegmentModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-lg">
        <div class="modal-content shadow-lg">
            ...
            <form action="../server/app.php" method="post" id="createSegmentForm">
                <?= csrf_field() ?>
...
function updateSegmentPreview() {
    ...
    segmentPreviewDebounce = setTimeout(function () {
        ...
        fetch('../server/segment_tools.php?' + params.toString())
            .then(r => r.json())
            .then(data => {
