...
## Planification et automatisations de campagnes (§30, Phase 2)
...
normalement) ; une campagne "récurrente" (renouvellement automatique
quotidien/hebdomadaire/mensuel) génère de la même façon sa prochaine
occurrence via une tâche de fond, jamais depuis une requête HTTP :

- Si `bin/process-campaign.php --daemon` tourne déjà (recommandé ci-dessus),
  **rien à faire de plus** : il promeut les campagnes planifiées dues ET
  génère les occurrences récurrentes dues à chaque tour de boucle (~3 s).
...
```
(ce script fait les deux : planification ET automatisations.)

Sans l'un ou l'autre, une campagne programmée n'est **jamais** lancée
automatiquement, et une campagne récurrente ne se renouvelle **jamais** —
seul un clic manuel sur "Lancer maintenant" ferait partir la campagne
programmée.
